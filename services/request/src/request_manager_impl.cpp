...
bool RequestManagerImpl::Start(const std::shared_ptr<IRequest> &request)
{
    ...
    if (Get(requestId) != nullptr) {
        IAM_LOGE("request already exists, requestId:%{public}d", requestId);
        return false;
    }
    ...
    if (subsequentSameTypeCount > request->GetMaxConcurrency()) {
        IAM_LOGE("request max concurrency reached, requestId:%{public}d", requestId);
        waitingRequests_.push_back(request);
        return true;
    }

    runningRequests_.push_back(request);
    TaskRunnerManager::GetInstance().PostTaskOnResident([request]() { request->Start(); });
    return true;
}
...
bool RequestManagerImpl::CancelRequestByScheduleId(ScheduleId scheduleId)
{
    ...
    IAM_LOGE("request not found, scheduleId:%" PRIu64, scheduleId);
    return false;
}
...
void RequestManagerImpl::Remove(RequestId requestId)
{
    ...
    for (auto it = runningRequests_.begin(); it != runningRequests_.end(); ++it) {
        if (*it != nullptr && (*it)->GetRequestId() == requestId) {
            removedRunningType = (*it)->GetRequestType();
    ...
    if (itToRemove != waitingRequests_.end()) {
        requestToStart = *itToRemove;
        waitingRequests_.erase(itToRemove);
...
{
    for (const auto &request : runningRequests_) {
        if (request != nullptr && request->GetRequestId() == requestId) {
            return request;
        }
    }
