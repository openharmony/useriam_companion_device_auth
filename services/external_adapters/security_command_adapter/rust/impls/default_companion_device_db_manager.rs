...
use crate::common::constants::{
    ErrorCode, ExecutorSecurityLevel, ProcessorType, TrackAbilityLevel, SECURE_RANDOM_MAX_ATTEMPTS, SHARE_KEY_LEN,
};
use crate::traits::companion_device_db_manager::{CompanionDeviceDbManager, CompanionDeviceFilter};
...
pub const COMPANION_DEVICE_CAPABILITY: &str = "companion_device_capability";
pub const COMPANION_DEVICE_SK: &str = "companion_device_sk";
...
pub struct DefaultCompanionDeviceDbManager {
    ...
    pub companion_token_infos: Vec<CompanionDeviceToken>,
}

impl DefaultCompanionDeviceDbManager {
    pub fn new() -> Self {
        DefaultCompanionDeviceDbManager {
    ...
    fn get_token_index_by_template_info(&self, template_id: u64, processor_type: ProcessorType) -> Option<usize> {
        self.companion_token_infos
            .iter()
            .position(|token| token.template_id == template_id && token.processor_type == processor_type)
    }
    ...
    fn serialize_device_capability_info(capability_infos: &[CompanionDeviceCapability], parcel: &mut Parcel) {
        ...
        for capability_info in capability_infos {
            parcel.write_i32(capability_info.processor_type as i32);
    ...
    fn deserialize_device_capability_info(parcel: &mut Parcel) -> Result<Vec<CompanionDeviceCapability>, ErrorCode> {
        ...
        for _ in 0..count {
            ...
            let device_type = ProcessorType::try_from(device_type_value).map_err(|e| p!(e))?;
            ...
            let capability_info = CompanionDeviceCapability { processor_type: device_type, esl, track_ability_level };
    ...
    fn serialize_device_sk(sk_infos: &[CompanionDeviceSk], parcel: &mut Parcel) {
        ...
        for sk_info in sk_infos {
            parcel.write_i32(sk_info.processor_type as i32);
...
impl CompanionDeviceDbManager for DefaultCompanionDeviceDbManager {
    fn add_device(
        &mut self,
    ...
    fn add_token(&mut self, token_info: &CompanionDeviceToken) -> Result<(), ErrorCode> {
        ...
        match self.get_token_index_by_template_info(token_info.template_id, token_info.processor_type) {
            Some(index) => {
    ...
    fn get_token(&self, template_id: u64, processor_type: ProcessorType) -> Result<CompanionDeviceToken, ErrorCode> {
        ...
        self.get_token_index_by_template_info(template_id, processor_type)
            .map(|index| self.companion_token_infos[index].clone())
            .ok_or_else(|| {
                log_e!(
                    "Token not found for template_id: {:x}, processor_type: {:?}",
                    template_id as u16,
                    processor_type
                );
                ErrorCode::NotFound
    ...
    fn remove_token(
        ...
        self.get_token_index_by_template_info(template_id, processor_type)
            .map(|index| {
                ...
                log_i!(
                    "Token removed successfully for template_id: {:x}, processor_type: {:?}",
                    template_id as u16,
                    processor_type
                );
                token
    ...
    fn update_token(&mut self, token_info: &CompanionDeviceToken) -> Result<(), ErrorCode> {
    ...
    fn read_device_db(&mut self) -> Result<(), ErrorCode> {
        ...
        let device_data: Vec<u8> = StorageIoRegistry::get().read(COMPANION_DEVICE_DB).map_err(|e| p!(e))?;
    ...
    fn read_device_profile(&self, template_id: u64) -> Result<CompanionDeviceProfile, ErrorCode> {
        log_i!("read_device_profile start, template_id:{:04x}", template_id as u16);
    ...
    fn write_device_profile(&self, template_id: u64, base_info: &CompanionDeviceProfile) -> Result<(), ErrorCode> {
        log_i!("write_device_profile start, template_id:{:04x}", template_id as u16);
        let filename = format!("{:x}_{}", template_id, COMPANION_DEVICE_PROFILE);
        let mut parcel = Parcel::new();
    ...
    fn delete_device_profile(&self, template_id: u64) -> Result<(), ErrorCode> {
        log_i!("delete_device_profile start, template_id:{:04x}", template_id as u16);
        let filename = format!("{:x}_{}", template_id, COMPANION_DEVICE_PROFILE);
...
impl Default for DefaultCompanionDeviceDbManager {
    fn default() -> Self {
        Self::new()
    }
}
