...
pub const HOST_BINDING_SK: &str = "host_binding_sk";
pub const HOST_BINDING_TOKEN: &str = "host_binding_token";
...
impl DefaultHostBindingDbManager {
    ...
    fn deserialize_device_sk(parcel: &mut Parcel) -> Result<HostBindingSk, ErrorCode> {
        let version = parcel.read_i32().map_err(|e| p!(e))?;
        if version > CURRENT_VERSION {
            log_e!("db_version is error, db_version:{}, current_version:{}", version, CURRENT_VERSION);
            return Err(ErrorCode::GeneralError);
    ...
    fn write_device_db(&self) -> Result<(), ErrorCode> {
        ...
        StorageIoRegistry::get().write(HOST_BINDING_DB, parcel.as_slice()).map_err(|e| p!(e))?;
        Ok(())
    }
...
impl HostBindingDbManager for DefaultHostBindingDbManager {
    ...
    fn read_device_db(&mut self) -> Result<(), ErrorCode> {
        ...
        let device_infos: Vec<u8> = StorageIoRegistry::get().read(HOST_BINDING_DB).map_err(|e| p!(e))?;
