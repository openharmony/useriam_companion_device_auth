...
#define LOG_FILE_ID LOG_FILE_SOFT_BUS_CHANNEL
...
bool SoftBusChannel::Initialize()
{
    ...
    deviceResyncScheduler_ = DeviceResyncScheduler::Create(deviceStatusManager_);
...
bool SoftBusChannel::Start()
{
    ...
    if (!deviceResyncScheduler_->Start()) {
        IAM_LOGE("DeviceResyncScheduler start failed");
        return false;
    }
    started_ = true;
