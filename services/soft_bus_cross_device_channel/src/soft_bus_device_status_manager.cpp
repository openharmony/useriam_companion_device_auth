...
bool SoftBusDeviceStatusManager::Initialize()
{
    ...
    ENSURE_OR_RETURN_VAL(localUdid.has_value(), false);
    localDeviceId_ = localUdid.value();
...
bool SoftBusDeviceStatusManager::InitDeviceManager()
{
    ...
    auto dmInitCallback = MakeShared<IamDmInitCallback>();
    ENSURE_OR_RETURN_VAL(dmInitCallback != nullptr, false);

    int32_t ret = DeviceManager::GetInstance().InitDeviceManager(PKG_NAME, dmInitCallback);
    if (ret != 0) {
        IAM_LOGE("InitDeviceManager error, result: %{public}d", ret);
        return false;
    }

    dmInitialized_ = true;
    dmInitCallback_ = dmInitCallback;
...
void SoftBusDeviceStatusManager::UnInitDeviceManager()
{
    ...
    int32_t ret = DeviceManager::GetInstance().UnInitDeviceManager(PKG_NAME);
    if (ret != 0) {
...
bool SoftBusDeviceStatusManager::QueryTrustedDevices(std::vector<DmDeviceInfo> &deviceList)
{
    ...
    int32_t dmRet = dm.GetTrustedDeviceList(PKG_NAME, "", deviceList);
    if (dmRet != 0) {
        IAM_LOGE("GetTrustedDeviceList failed: %{public}d", dmRet);
        return false;
    }

    IAM_LOGE("trusted devices num: %{public}zu", deviceList.size());
    auto it = std::remove_if(deviceList.begin(), deviceList.end(), [](const DmDeviceInfo &device) {
        ...
        if (!IsDeviceTypeIdSupport(deviceTypeId)) {
            IAM_LOGE("device type not supported: %{public}d", static_cast<uint32_t>(deviceTypeId));
            return true;
        }
        return false;
    });
    ...
    std::vector<PhysicalDeviceStatus> &retPhysicalDeviceStatuses)
{
    ...
    for (const auto &device : deviceList) {
        ...
        std::string deviceId;
        int32_t ret = dm.GetUdidByNetworkId(PKG_NAME, networkId, deviceId);
        if (ret != 0 || deviceId.empty()) {
            IAM_LOGE("GetUdidByNetworkId failed for networkId %{public}s, ret %{public}d",
                GetMaskedString(networkId).c_str(), ret);
            continue;
        }
...
PhysicalDeviceKey SoftBusDeviceStatusManager::GetLocalPhysicalDeviceKey() const
{
    PhysicalDeviceKey key;
    key.idType = DeviceIdType::UNIFIED_DEVICE_ID;
    key.deviceId = localDeviceId_;
    return key;
}
...
bool SoftBusDeviceStatusManager::RegisterDeviceStatusCallback()
{
    ...
    auto &dm = DeviceManager::GetInstance();
    int32_t ret = dm.RegisterDevStatusCallback(PKG_NAME, "", dsCallback);
    if (ret != 0) {
