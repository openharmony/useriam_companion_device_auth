...
public:
    void SetUp() override
    {
        ...
        ON_CALL(*mockChannel_, SubscribeConnectionStatus(_)).WillByDefault(Return(ByMove(MakeSubscription())));
        ON_CALL(*mockChannel_, SubscribeIncomingConnection(_)).WillByDefault(Return(ByMove(MakeSubscription())));
        ON_CALL(*mockChannel_, SubscribeRawMessage(_)).WillByDefault(Return(ByMove(MakeSubscription())));
        ON_CALL(*mockChannel_, SubscribePhysicalDeviceStatus(_)).WillByDefault(Return(ByMove(MakeSubscription())));
...
HWTEST_F(CrossDeviceCommManagerImplTest, GetLocalDeviceStatus_001, TestSize.Level0)
{
    std::vector<std::shared_ptr<ICrossDeviceChannel>> channels = { mockChannel_ };
    auto manager = CrossDeviceCommManagerImpl::Create(channels);
    ASSERT_NE(manager, nullptr);
...
HWTEST_F(CrossDeviceCommManagerImplTest, SubscribeLocalDeviceStatus_001, TestSize.Level0)
{
    std::vector<std::shared_ptr<ICrossDeviceChannel>> channels = { mockChannel_ };
    auto manager = CrossDeviceCommManagerImpl::Create(channels);
    ASSERT_NE(manager, nullptr);
