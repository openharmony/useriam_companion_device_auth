...
LocalDeviceStatus MakeLocalDeviceStatus(bool isAuthMaintainActive = true)
{
    LocalDeviceStatus status;
    status.isAuthMaintainActive = isAuthMaintainActive;
    status.deviceName = "LocalDevice";
    status.deviceUserName = "LocalUser";
    status.deviceModelInfo = "LocalModel";
    return status;
}
...
public:
    void SetUp() override
    {
        ...
        ON_CALL(mockCrossDeviceCommManager_, SubscribeLocalDeviceStatus(_))
            .WillByDefault(Return(ByMove(MakeSubscription())));
        ON_CALL(mockCrossDeviceCommManager_, GetLocalDeviceStatus()).WillByDefault(Return(MakeLocalDeviceStatus()));
...
HWTEST_F(HostBindingTest, HandleLocalDeviceStatusChanged_001, TestSize.Level0)
{
    auto persistedStatus = MakePersistedStatus(12345, 100, "test_device_id", 200);
    auto binding = HostBinding::Create(persistedStatus);
    ASSERT_NE(nullptr, binding);

    auto localStatus = MakeLocalDeviceStatus(true);
    binding->HandleLocalDeviceStatusChanged(localStatus);

    EXPECT_TRUE(binding->GetStatus().localAuthMaintainActive);
}

HWTEST_F(HostBindingTest, HandleLocalDeviceStatusChanged_002, TestSize.Level0)
{
    auto persistedStatus = MakePersistedStatus(12345, 100, "test_device_id", 200);
    auto binding = HostBinding::Create(persistedStatus);
    ASSERT_NE(nullptr, binding);
    ...
    auto localStatus = MakeLocalDeviceStatus(false);
    binding->HandleLocalDeviceStatusChanged(localStatus);
...
HWTEST_F(HostBindingTest, HandleLocalDeviceStatusChanged_003, TestSize.Level0)
{
    auto persistedStatus = MakePersistedStatus(12345, 100, "test_device_id", 200);
    auto binding = HostBinding::Create(persistedStatus);
    ASSERT_NE(nullptr, binding);
