...
fn mock_set_host_db_manager_for_host_update_token() {
    ...
    mock_host_db_manager.expect_read_device_capability_info()
        .returning(|| Ok(vec![create_mock_companion_device_capability()]));
    mock_host_db_manager.expect_get_token()
    ...
    mock_host_db_manager.expect_get_token().returning(|| Err(ErrorCode::GeneralError));
...
fn init_test_success() {
    ...
    mock_crypto_engine.expect_generate_ed25519_key_pair().returning(|| Ok(create_mock_key_pair()));
...
fn init_test_misc_manager_fail() {
    ...
    let mut mock_misc_manager = MockMiscManager::new();
    mock_misc_manager.expect_set_local_key_pair().returning(|| Err(ErrorCode::GeneralError));
    MiscManagerRegistry::set(Box::new(mock_misc_manager));
...
fn init_test_host_db_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_read_device_db().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn get_executor_info_test_success() {
    ...
    let mut output = GetExecutorInfoOutputFfi {
        esl: 0,
        max_template_acl: 0,
...
fn host_on_register_finish_test_success() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_get_device_list().returning(|| vec![create_mock_companion_device_info(123)]);
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_on_register_finish_test_misc_manager_fail() {
    ...
    let mut mock_misc_manager = MockMiscManager::new();
    mock_misc_manager.expect_set_fwk_pub_key().returning(|| Err(ErrorCode::GeneralError));
    MiscManagerRegistry::set(Box::new(mock_misc_manager));
...
fn host_on_register_finish_test_remove_device_success() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_get_device_list().returning(|| vec![create_mock_companion_device_info(123)]);
    mock_host_db_manager.expect_remove_device().returning(|| Ok(create_mock_companion_device_info(123)));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_on_register_finish_test_remove_device_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_get_device_list().returning(|| vec![create_mock_companion_device_info(123)]);
    mock_host_db_manager.expect_remove_device().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_get_persisted_status_test_success_with_devices() {
    ...
    let mut output = HostGetPersistedStatusOutputFfi {
        companion_status_list: CompanionStatusArrayFfi::default(),
    };
...
fn host_get_persisted_status_test_read_device_base_info_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_get_device_list().returning(|| vec![create_mock_companion_device_info(123)]);
    mock_host_db_manager.expect_read_device_base_info().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_begin_companion_check_test_add_request_fail() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_add_request().returning(|| Err(ErrorCode::GeneralError));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_end_companion_check_test_success() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(sync_status_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_end_companion_check_test_remove_request_fail() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(|| Err(ErrorCode::GeneralError));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_end_companion_check_test_request_end_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_read_device_capability_info().returning(|| Ok(Vec::new()));
    mock_host_db_manager.expect_get_device().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_cancel_companion_check_test_fail() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(|| Err(ErrorCode::NotFound));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_begin_add_companion_test_success() {
    ...
    let nego_input = HostGetInitKeyNegotiationInputFfi {
        request_id: 1,
        secure_protocol_id: 1,
    };
    let enroll_request = HostDeviceEnrollRequest::new(&nego_input).unwrap();
    HostRequestManagerRegistry::get_mut().add_request(Box::new(enroll_request)).unwrap();
    ...
    let sec_key_nego_reply = SecKeyNegoReply {
        algorithm: 1,
        challenge: 0,
        pub_key: vec![1, 2, 3],
    };
...
fn host_end_add_companion_test_success() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(enroll_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_end_add_companion_test_request_end_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_generate_unique_template_id().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_remove_companion_test_success() {
    ...
    let mut output = HostRemoveCompanionOutputFfi {
        user_id: 0,
        companion_device_key: DeviceKeyFfi::default(),
    };
...
fn host_begin_issue_token_test_success() {
    ...
    let pre_input = HostPreIssueTokenInputFfi {
        request_id: 1,
        template_id: 123,
        fwk_message: DataArray1024Ffi::default(),
    };
    ...
    HostRequestManagerRegistry::get_mut().add_request(Box::new(issue_token_request)).unwrap();

    let input = HostBeginIssueTokenInputFfi {
        request_id: 1,
        secure_protocol_id: 1,
        sec_message: DataArray1024Ffi::default(),
    };
...
fn host_begin_issue_token_test_request_begin_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_read_device_capability_info().returning(|| Err(ErrorCode::GeneralError));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_end_issue_token_test_success() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(issue_token_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));

    let input = HostEndIssueTokenInputFfi {
        request_id: 1,
        secure_protocol_id: 1,
        sec_message: DataArray1024Ffi::default(),
    };
...
fn host_end_token_auth_test_success() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(token_auth_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
...
fn host_revoke_token_test_success() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_remove_token()
        .returning(|| Ok(create_mock_companion_token_info(AuthTrustLevel::Atl3)));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_revoke_token_test_fail() {
    ...
    let mut mock_host_db_manager = MockHostDbManager::new();
    mock_host_db_manager.expect_remove_token().returning(|| Err(ErrorCode::NotFound));
    HostDbManagerRegistry::set(Box::new(mock_host_db_manager));
...
fn host_update_companion_enabled_business_ids_test_success() {
    ...
    let input = HostUpdateCompanionEnabledBusinessIdsInputFfi {
        template_id: 123,
        business_ids: Int32Array64Ffi::default(),
    };
...
fn host_end_delegate_auth_test_success() {
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(delegate_auth_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
    ...
    let input = HostEndDelegateAuthInputFfi {
        request_id: 1,
        secure_protocol_id: 1,
        sec_message: DataArray1024Ffi::default(),
    };
    let mut output = HostEndDelegateAuthOutputFfi {
        fwk_message: DataArray1024Ffi::default(),
        auth_type: 0,
        atl: 0,
    };
...
fn host_process_pre_obtain_token_test_success() {
    ...
    let input = HostProcessPreObtainTokenInputFfi {
        request_id: 1,
        template_id: 123,
        secure_protocol_id: 1,
    };
...
fn host_process_obtain_token_test_success() {
    ...
    let pre_input = HostProcessPreObtainTokenInputFfi {
        request_id: 1,
        template_id: 123,
        secure_protocol_id: 1,
    };
    ...
    let mut mock_host_request_manager = MockHostRequestManager::new();
    mock_host_request_manager.expect_remove_request().returning(move || Ok(Box::new(obtain_token_request.clone())));
    HostRequestManagerRegistry::set(Box::new(mock_host_request_manager));
    ...
    let mut output = HostProcessObtainTokenOutputFfi {
        sec_message: DataArray1024Ffi::default(),
        atl: 0,
    };
...
fn host_update_token_test_success() {
    ...
    let input = HostUpdateTokenInputFfi {
        template_id: 123,
        fwk_message: DataArray1024Ffi::try_from(&fwk_message).unwrap(),
    };
...
fn companion_get_persisted_status_test_success() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_get_device_list().returning(|| vec![create_mock_host_device_info(123)]);
    ...
    let mut output = CompanionGetPersistedStatusOutputFfi {
        binding_status_list: HostBindingStatusArrayFfi::default(),
    };
...
fn companion_get_persisted_status_test_token_valid_fail() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_get_device_list().returning(|| vec![create_mock_host_device_info(123)]);
    mock_companion_db_manager.expect_is_device_token_valid().returning(|| Err(ErrorCode::GeneralError));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
...
fn companion_process_check_test_request_begin_fail() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_read_device_sk().returning(|| Err(ErrorCode::GeneralError));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
...
fn companion_init_key_negotiation_test_add_request_fail() {
    ...
    let mut mock_companion_request_manager = MockCompanionRequestManager::new();
    mock_companion_request_manager.expect_add_request().returning(|| Err(ErrorCode::GeneralError));
    CompanionRequestManagerRegistry::set(Box::new(mock_companion_request_manager));
...
fn companion_begin_add_host_binding_test_success() {
    ...
    mock_companion_db_manager.expect_get_device_by_binding_id().returning(|| Ok(create_mock_host_device_info(1)));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
    ...
    CompanionRequestManagerRegistry::get_mut().add_request(Box::new(enroll_request)).unwrap();
...
fn companion_end_add_host_binding_test_success() {
    ...
    let mut mock_companion_request_manager = MockCompanionRequestManager::new();
    mock_companion_request_manager.expect_remove_request().returning(move || Ok(Box::new(enroll_request.clone())));
    CompanionRequestManagerRegistry::set(Box::new(mock_companion_request_manager));

    let sec_issue_token = SecIssueToken {
        challenge: 0,
        atl: 30000,
        token: Vec::new(),
    };
    let sec_message = sec_issue_token.encrypt_issue_token(
        &[0u8; SALT_LEN_FFI], DeviceType::None, &[1, 2, 3]).unwrap();
...
fn companion_end_add_host_binding_test_remove_request_fail() {
    ...
    let mut mock_companion_request_manager = MockCompanionRequestManager::new();
    mock_companion_request_manager.expect_remove_request().returning(|| Err(ErrorCode::NotFound));
    CompanionRequestManagerRegistry::set(Box::new(mock_companion_request_manager));
...
fn companion_remove_host_binding_test_fail() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_remove_device().returning(|| Err(ErrorCode::NotFound));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
...
fn companion_process_token_auth_test_success() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_read_device_sk().returning(|| Ok(HostDeviceSk { sk: Vec::new() }));
    mock_companion_db_manager.expect_read_device_token().returning(|| Ok(HostTokenInfo {
        token: Vec::new(),
        atl: AuthTrustLevel::Atl3,
    }));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
...
fn companion_revoke_token_test_fail() {
    ...
    let mut mock_companion_db_manager = MockCompanionDbManager::new();
    mock_companion_db_manager.expect_delete_device_token().returning(|| Err(ErrorCode::NotFound));
    CompanionDbManagerRegistry::set(Box::new(mock_companion_db_manager));
...
fn companion_begin_obtain_token_test_success() {
    ...
    let sec_pre_obtain_token_request = SecPreObtainTokenRequest {
        salt: [0u8; HKDF_SALT_SIZE],
        challenge: 0,
    };
