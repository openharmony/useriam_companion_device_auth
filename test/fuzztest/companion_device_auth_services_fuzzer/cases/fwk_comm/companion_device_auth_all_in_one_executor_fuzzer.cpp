...
using FuzzFunction = void (*)(std::shared_ptr<CompanionDeviceAuthAllInOneExecutor> &exec, FuzzedDataProvider &);
...
{
    auto exec = std::make_shared<CompanionDeviceAuthAllInOneExecutor>();
    if (!exec) {
        return;
    }
