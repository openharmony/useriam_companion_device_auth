...
static void FuzzShouldCancelOnNewRequest(std::shared_ptr<HostSingleMixAuthRequest> &request,
    FuzzedDataProvider &fuzzData)
{
...
static void FuzzCancel(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    ResultCode result = GenerateFuzzResultCode(fuzzData);
    (void)request->Cancel(result);
}

static void FuzzCompleteWithSuccess(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
...
static void FuzzCompleteWithError(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    ResultCode result = GenerateFuzzResultCode(fuzzData);
    (void)request->CompleteWithError(result);
}

static void FuzzDestroy(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
    (void)request->Destroy();
}

static void FuzzInvokeCallback(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    ResultCode result = GenerateFuzzResultCode(fuzzData);
    std::vector<uint8_t> extraInfo =
        fuzzData.ConsumeBytes<uint8_t>(fuzzData.ConsumeIntegralInRange<size_t>(0, FUZZ_MAX_FWK_MESSAGE_LENGTH));
...
static void FuzzHandleTokenAuthResult(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    ResultCode result = GenerateFuzzResultCode(fuzzData);
    std::vector<uint8_t> extraInfo =
        fuzzData.ConsumeBytes<uint8_t>(fuzzData.ConsumeIntegralInRange<size_t>(0, FUZZ_MAX_MESSAGE_LENGTH));
    (void)request->HandleTokenAuthResult(result, extraInfo);
}

static void FuzzHandleDelegateAuthResult(std::shared_ptr<HostSingleMixAuthRequest> &request,
    FuzzedDataProvider &fuzzData)
{
    ResultCode result = GenerateFuzzResultCode(fuzzData);
    std::vector<uint8_t> extraInfo =
        fuzzData.ConsumeBytes<uint8_t>(fuzzData.ConsumeIntegralInRange<size_t>(0, FUZZ_MAX_MESSAGE_LENGTH));
    (void)request->HandleDelegateAuthResult(result, extraInfo);
}

static void FuzzGetRequestInfo(std::shared_ptr<HostSingleMixAuthRequest> &request, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
...
static const HostSingleMixAuthRequestFuzzFunction g_fuzzFuncs[] = {
    ...
    FuzzDestroy,
    FuzzInvokeCallback,
    FuzzHandleTokenAuthResult,
    FuzzHandleDelegateAuthResult,
    FuzzGetRequestInfo,
};
...
void FuzzHostSingleMixAuthRequest(FuzzedDataProvider &fuzzData)
{
    ...
    auto request =
        std::make_shared<HostSingleMixAuthRequest>(scheduleId, fwkMsg, hostUserId, templateId, std::move(callback));
    if (!request) {
        return;
    }
...
FUZZ_REGISTER(FuzzHostSingleMixAuthRequest)
