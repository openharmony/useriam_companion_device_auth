...
static void FuzzGetMessageType(std::shared_ptr<CompanionPreIssueTokenHandler> &handler, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
...
static void FuzzRegister(std::shared_ptr<CompanionPreIssueTokenHandler> &handler, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
    handler->Register();
}

static void FuzzHandleIncomingMessage(std::shared_ptr<CompanionPreIssueTokenHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
...
static void FuzzHandleRequest(std::shared_ptr<CompanionPreIssueTokenHandler> &handler, FuzzedDataProvider &fuzzData)
{
    Attributes request = GenerateFuzzAttributes(fuzzData);
    auto replyCallback = [](const Attributes &reply) { (void)reply; };
    OnMessageReply onMessageReply = replyCallback;
    handler->HandleRequest(request, onMessageReply);
}

static void FuzzHandleRequestWithEmptyAttrs(std::shared_ptr<CompanionPreIssueTokenHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
...
static void FuzzHandleRequestWithLargeAttrs(std::shared_ptr<CompanionPreIssueTokenHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
...
static void FuzzCreateNewHandler(std::shared_ptr<CompanionPreIssueTokenHandler> &handler, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
    auto newHandler = std::make_shared<CompanionPreIssueTokenHandler>();
    (void)newHandler;
    (void)handler;
}
...
static void FuzzHandleIncomingMessageWithNullCallback(std::shared_ptr<CompanionPreIssueTokenHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    Attributes request = GenerateFuzzAttributes(fuzzData);
...
static const CompanionPreIssueTokenHandlerFuzzFunction g_fuzzFuncs[] = {
    FuzzGetMessageType,
    FuzzRegister,
...
constexpr uint8_t NUM_FUZZ_OPERATIONS = sizeof(g_fuzzFuncs) / sizeof(CompanionPreIssueTokenHandlerFuzzFunction);

void FuzzCompanionPreIssueTokenHandler(FuzzedDataProvider &fuzzData)
{
    auto handler = std::make_shared<CompanionPreIssueTokenHandler>();
    if (!handler) {
        return;
    }
