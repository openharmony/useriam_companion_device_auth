...
static void FuzzRegister(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler, FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
    handler->Register();
}

static void FuzzHandleIncomingMessage(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
...
static void FuzzHandleRequest(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    Attributes request = GenerateFuzzAttributes(fuzzData);
    auto replyCallback = [](const Attributes &reply) { (void)reply; };
...
static void FuzzHandleRequestWithEmptyAttrs(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
...
static void FuzzHandleRequestWithLargeAttrs(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
...
static void FuzzCreateNewHandler(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    (void)fuzzData;
    auto newHandler = std::make_shared<CompanionInitKeyNegotiationHandler>();
    (void)newHandler;
    (void)handler;
}
...
static void FuzzHandleIncomingMessageWithNullCallback(std::shared_ptr<CompanionInitKeyNegotiationHandler> &handler,
    FuzzedDataProvider &fuzzData)
{
    Attributes request = GenerateFuzzAttributes(fuzzData);
...
{
    auto handler = std::make_shared<CompanionInitKeyNegotiationHandler>();
    if (!handler) {
        return;
    }
